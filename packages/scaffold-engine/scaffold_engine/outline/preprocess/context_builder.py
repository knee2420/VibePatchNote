...
from scaffold_engine.utils.coordinates import normalize_bbox
        ...
        for page_idx in range(total_pages):
            ...
            try:
                ...
                if tabs and len(tabs.tables) > 0:
                    ...
                    for t_idx, tab in enumerate(tabs, 1):
                        norm_box = normalize_bbox(tab.bbox, pw, ph)
                        tables_meta.append({
                            "table_id": t_idx,
                            "norm_bbox": norm_box,
                            "cols": tab.col_count,
            ...
            try:
                for img_idx, img_info in enumerate(page.get_image_info(xrefs=True), 1):
                    b = img_info.get("bbox")
                    if b:
                        norm_box = normalize_bbox(b, pw, ph)
            ...
            for b in raw_blocks:
                if b.get("type") == 0:
                    ...
                    b_text = b_text.strip()
                    if b_text:
                        norm_box = normalize_bbox(b.get("bbox", [0, 0, 0, 0]), pw, ph)
                        text_blocks.append({
                            "text": b_text,
                            "size": round(b_size, 1),
                            "norm_bbox": norm_box,
