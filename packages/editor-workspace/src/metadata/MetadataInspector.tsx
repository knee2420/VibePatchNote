...
export function MetadataInspector({
  ...
  return (
    <div className={`w-full h-full p-4 flex flex-col gap-4 overflow-y-auto select-none text-xs ${className}`}>
      ...
      <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800 flex flex-col gap-2.5">
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-400 font-mono">
          <span>현재: <strong className="text-slate-200">{currentCount.toLocaleString()}</strong>자</span>
          <div className="flex items-center gap-1">
            ...
            <input
              ...
              className="w-16 bg-slate-950 border border-slate-700 rounded px-1.5 py-0.5 text-right text-slate-200 outline-none focus:border-purple-500"
            />
            <span>자</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        <div className="flex flex-col gap-1.5">
          <label className="text-[11px] font-bold text-slate-400 uppercase">진행 상태</label>
          <select
            value={metadata.status || statusOptions[0]}
            onChange={(e) => onChange?.({ ...metadata, status: e.target.value })}
            className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-200 outline-none focus:border-purple-500"
          >
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[11px] font-bold text-slate-400 uppercase">라벨 구분</label>
          <select
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-1.5">
          <input
            ...
            className="flex-1 bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1 text-xs text-slate-200 outline-none focus:border-purple-500 placeholder-slate-600"
          />
          <button
            type="button"
        ...
        <div className="flex flex-wrap gap-1.5 mt-1">
          {(metadata.tags || []).map((tag) => (
            <span
              key={tag}
              className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-purple-950/40 text-purple-300 border border-purple-800/50 text-[10px]"
            >
              <span>{tag}</span>
              <button
                type="button"
      ...
      <div className="flex-1 flex flex-col gap-1.5 min-h-[120px]">
        ...
        <textarea
          ...
          className="w-full flex-1 min-h-[100px] bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs leading-relaxed text-slate-300 placeholder-slate-600 outline-none focus:border-purple-500 resize-none font-sans"
        />
      </div>
    </div>
  );
