...
export const ProviderExecutionBadge = memo(function ProviderExecutionBadge({
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-mono bg-slate-50 border border-slate-200 text-slate-600 select-none shadow-2xs ${className}`}
    ...
    >
      <div className="flex items-center gap-1 text-indigo-600">
        ...
        <span className="font-semibold text-slate-800">{info.model}</span>
      </div>
      ...
      {info.fallbackUsed && (
        <span className="flex items-center gap-0.5 text-amber-600 font-semibold" title="자동 폴백 모델 적용됨">
