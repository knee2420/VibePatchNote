...
export function PagedCanvasContainer({
  ...
  if (layoutMode === 'continuous') {
    return (
      <div className={`w-full h-full overflow-auto p-6 flex flex-col items-center select-text ${className}`}>
        <div
          style={{
  ...
  if (layoutMode === 'paged') {
    return (
      <div className={`w-full h-full overflow-auto p-8 flex flex-col items-center bg-slate-950/90 select-text ${className}`}>
        <div
          style={{
            transform: zoomScale !== 1 ? `scale(${zoomScale})` : undefined,
            transformOrigin: 'top center',
        ...
        >
          {/* A4 용지 프레임 */}
          <div className="w-[210mm] min-h-[297mm] bg-white rounded-xs shadow-2xl border border-slate-300/80 p-[20mm] flex flex-col justify-between text-slate-900 relative">
            {/* 상단 헤더 여백 */}
  ...
  if (layoutMode === 'spread') {
    return (
      <div className={`w-full h-full overflow-auto p-8 flex flex-col items-center bg-slate-950/90 select-text ${className}`}>
        ...
        >
          {/* 좌측 면 (Left Page) */}
          <div className="w-[180mm] min-h-[260mm] bg-white rounded-xs shadow-2xl border border-slate-300/80 p-[16mm] flex flex-col justify-between text-slate-900">
  ...
  // 4. 집중/젠 모드 (Zen Mode)
  return (
    <div className={`w-full h-full overflow-auto p-12 flex flex-col items-center bg-slate-950 select-text ${className}`}>
      <div
        style={{
          transform: zoomScale !== 1 ? `scale(${zoomScale})` : undefined,
          transformOrigin: 'top center',
        ...
        className="w-full max-w-3xl flex flex-col items-center shadow-2xl"
      >
        {children}
      </div>
    </div>
