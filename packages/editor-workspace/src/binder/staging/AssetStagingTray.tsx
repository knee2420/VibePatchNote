...
function AssetStagingTrayInner<T = Record<string, unknown>>({
  ...
  return (
    <div className={`flex flex-col bg-slate-50 border-t border-slate-200 transition-all ${className}`}>
      {/* 헤더 토글 바 */}
      <div
        onClick={() => isCollapsible && setIsExpanded(!isExpanded)}
        className="flex items-center justify-between px-3 py-2 bg-slate-50/90 cursor-pointer select-none hover:bg-slate-100 border-b border-slate-200"
      >
        <div className="flex items-center gap-2">
          <Inbox className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
          <span className="text-xs font-semibold text-slate-700">미매핑 에셋 인박스</span>
        ...
        <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
          {onAutoBindAll && assets.length > 0 && (
            <button
              ...
              className="flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-medium bg-indigo-600 hover:bg-indigo-500 text-white transition-colors cursor-pointer shadow-xs"
            >
          ...
          <div className="flex items-center gap-1.5">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="에셋 검색..."
                className="w-full pl-7 pr-2 py-1 text-[11px] rounded bg-white border border-slate-200 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 shadow-2xs"
              />
            </div>
              ...
              {(['all', 'text', 'table', 'image'] as const).map((t) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      px-1.5 py-0.5 rounded text-[10px] font-mono transition-colors uppercase cursor-pointer
                      ${
                        isActive
          ...
          <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 no-scrollbar min-h-[100px]">
            {filteredAssets.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-6 text-center text-slate-400">
                <p className="text-xs font-medium text-slate-500">미할당 대기 중인 에셋이 없습니다.</p>
