...
class DocumentOutlineArchiveAdapter:
    ...
    def archive(
        ...
        *,
        ...
        committed = self._artifacts.commit(meta.doc_id, KIND, artifact_files.to_commit_dict(), provenance)
        scope = current_scope()
        if scope:
            scope.set_label(
    ...
    def _response_from_artifact(
        ...
        result = OutlineExecutionResult(
            status="completed",
            doc_id=meta.doc_id,
            document_title=tree.get("document_title") or tree.get("documentTitle") or meta.original_name,
