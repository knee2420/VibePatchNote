            ...
            if pipeline_tel and run_id:
                if isinstance(pipeline_tel, dict):
                    ...
                    cache_span = {
                        ...
                        "data_via": ["extract_outline.py (load_adopted)", "local_document_artifact_repository.py (LocalDocumentArtifactRepository.load_head)"],
                        "start_time": cache_dt.isoformat(),
                        "end_time": (cache_dt + timedelta(milliseconds=2)).isoformat(),
                        "inputs": {"docId": doc_id, "force_refresh": force_refresh, "target_pointer": f"documents/{doc_id}/artifacts/outline/HEAD.json"},
                        "outputs": {"has_adopted": False, "action": "execute_pipeline", "cache_status": "MISS", "reason": "Fresh analysis required"},
                    ...
                    route_span = {
                        ...
                        "inputs": {"primary_provider": harness_provider, "target_model": harness_model, "routing_policy": "primary_with_quota_fallback"},
                        "outputs": {"status": "AVAILABLE", "routed_provider": harness_provider, "routed_engine": actual_harness.__class__.__name__, "quota_check": "NORMAL"},
                        "usage": {"latency_ms": 2.0, "total_tokens": 0, "prompt_tokens": 0, "completion_tokens": 0},
                        "duration_ms": 2.0,
                    }
                    ...
                    if status_val == "SUCCESS" and provenance:
                        ...
                        settle_span = {
                            ...
                            "inputs": {"run_id": run_id, "total_tokens": cost.total_tokens, "duration_seconds": pipeline_tel.get("cli_duration", 0)},
                            "outputs": {"status": "SETTLED", "run_id": run_id, "final_status": "SUCCESS", "ledger": "ledger.jsonl"},
