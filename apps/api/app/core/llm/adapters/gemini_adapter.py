...
CLI 를 거치지 않고 Google Generative Language REST API 를 직접 호출하여 고속 구조화 데이터를 추출합니다.
PDF 및 이미지 등 문서 파일이 첨부되면 멀티모달 inlineData(Base64) 파트로 자동 첨부하여 시각 비전 분석을 수행합니다.
호스트의 CredentialStore 가 API 키를 안전하게 관리합니다.
"""
...
class GoogleGenAiHarness(BaseLlmHarness):
    """Google Gemini Direct API 정규 어댑터.

    SDK 의존성을 추가하지 않아도 되도록 표준 `requests`로 구현합니다.
    API 키는 절대 로그나 텔레메트리에 노출되지 않습니다.
    ...
    def run_structured(
        ...
        **kwargs: Any,
        ...
        # 멀티모달 parts 구성: 첨부 파일이 있으면 inlineData 추가 후 text 프롬프트 결합
