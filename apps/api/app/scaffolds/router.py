...
ScaffoldArchiveServiceDep = Annotated[
...
@inject
async def get_archive_detail(scaffold_id: str, service: ScaffoldArchiveServiceDep):
...
async def save_archive_render(
    scaffold_id: str,
    payload: ScaffoldRenderUpdate,
    service: ScaffoldArchiveServiceDep,
):
...
async def save_archive_render_image(
    ...
    service: ScaffoldArchiveServiceDep = None,
):
    ...
    meta = service.update_render_image(scaffold_id, png_bytes)
    if not meta:
        raise HTTPException(status_code=404, detail="스캐폴드 아카이브를 찾을 수 없습니다.")
    return meta
