...
export function IdeReferenceDocDrawer({
  ...
  onSelectDoc,
  width = 580,
  leftOffset = 308, // 48px activity bar + ~260px primary sidebar
  onMouseDownResizer,
  ...
  const effectiveWidth = isMaximized ? Math.max(width, 800) : width;
  ...
  return (
    <div
      ...
      className="fixed top-9 bottom-6 z-40 bg-slate-900/98 backdrop-blur-xl border-r border-slate-800 shadow-2xl flex flex-col transition-all duration-200 select-none font-sans animate-in slide-in-from-left-4 duration-150"
    >
      {/* 우측 마우스 리사이저 핸들 (왼쪽 모달이므로 우측 테두리에 위치) */}
      {onMouseDownResizer && (
        <div
      ...
      <div className="px-3 py-1.5 border-b border-slate-800/60 bg-slate-925/40 flex items-center gap-1.5 overflow-x-auto text-[11px] font-mono scrollbar-none shrink-0">
        ...
        {referenceDocuments.slice(0, 5).map((doc) => (
          <button
            key={doc.id}
            type="button"
      ...
      {/* 3. 본문 뷰어 영역 (A4DocumentViewer 연동) */}
