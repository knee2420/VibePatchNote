    ...
    <div className="w-[340px] h-full flex flex-col border-l border-slate-200 bg-white dark:bg-slate-900 select-none text-slate-700 dark:text-slate-300 text-xs shrink-0 overflow-hidden nodrag">
      ...
      <div className="h-9 px-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50/70 dark:bg-slate-900/80 shrink-0">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          {isExtracting ? (
            <span className="text-[10px] font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/70 px-1.5 py-0.5 rounded animate-pulse">
              분석 중...
            </span>
          ) : (
        ...
        <div className="flex items-center gap-0.5 text-slate-400">
          ...
          {onRefresh && (
            ...
            >
              <RotateCw className={`w-3.5 h-3.5 ${isRefreshing || isExtracting ? 'animate-spin text-indigo-600' : ''}`} />
            </button>
          )}
          <button
            onClick={onClose}
      ...
      {/* 4. 파일 디렉토리 트리 본문 (VSCode / Obsidian Tree View or Skeleton) */}
      <div className="flex-1 overflow-y-auto overflow-x-hidden py-1">
        {isExtracting && outlines.length === 0 ? (
          /* 스켈레톤 목차 트리 뼈대 (Zero-Latency 대기 피드백) */
          <div className="p-3 space-y-3 animate-pulse">
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded bg-indigo-200/70 dark:bg-indigo-900/60" />
              <div className="h-3 w-40 rounded bg-indigo-200/70 dark:bg-indigo-900/60" />
            </div>
            ...
            <div className="flex items-center gap-2 pt-2">
              <div className="w-4 h-4 rounded bg-indigo-200/70 dark:bg-indigo-900/60" />
              <div className="h-3 w-36 rounded bg-indigo-200/70 dark:bg-indigo-900/60" />
            </div>
            <div className="pl-4 space-y-2">
              <div className="flex items-center gap-2">
                <div className="w-3.5 h-3.5 rounded bg-slate-200 dark:bg-slate-800" />
                <div className="h-2.5 w-44 rounded bg-slate-200 dark:bg-slate-800" />
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3.5 h-3.5 rounded bg-slate-200 dark:bg-slate-800" />
