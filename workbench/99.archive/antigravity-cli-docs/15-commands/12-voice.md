...
source: "https://antigravity.google/docs/cli/commands/voice/"
section: "Commands"
---
...
**Caution:** "Voice dictation requires an updated authentication permission granted at sign-in. If your active session was authenticated before voice dictation became available, transcription will fail." Run `/logout`, sign in again, and retry.
...
**Note:** "The `F5` key is remappable. Use `/keybindings` to bind `voice.start_dictation` to a different shortcut; the on-screen hint will automatically update to reflect your custom keybinding."
...
**Caution:** "Keep `mic-serve` on `localhost` (`127.0.0.1`). The microphone server is intended for local connections forwarded through SSH tunnels."
...
- **[Prompting & Interaction](https://antigravity.google/docs/cli/prompting)**: Multiline editing, interrupts, and pasting media into the prompt.
- **[Installation & Auth](https://antigravity.google/docs/cli/install)**: Sign-in flows, including the remote SSH OAuth loop.
