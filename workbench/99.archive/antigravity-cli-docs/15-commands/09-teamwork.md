...
source: "https://antigravity.google/docs/teamwork/"
section: "Commands"
---
...
**Note:** The `/teamwork-preview` command is available on paid plans across Google Antigravity 2.0 and the Antigravity CLI.
...
- **Sentinel**: The coordinator that takes over once you approve the prompt. Sentinel records your request, routes tasks, posts periodic progress updates, and spawns the Success Auditor before wrapping up.
...
- **Success Auditor**: The final reviewer spawned by Sentinel. It runs a full end-to-end verification pass and must confirm everything works before Sentinel presents the finished project to you.
...
|---|---|---|
| **General (Distributed coding)** | Multi-file SWE, large-scale refactoring, systems simulation, and research. | *Default* (automatically selected). |
| **Iterative coding (lighter SWE tasks)** | One self-contained change — a bug fix, a small feature, or a contained refactor. This path cannot decompose the task. | Explicit prompt signal (e.g., *"keep it small"*, *"keep it focused"*). |
| **Document Review** | Multi-angle critique and synthesis of papers, RFCs, and design docs. | Review requests (e.g., *"review this paper"*, *"critique this design doc"*). |
| **Math / Proof** | Mathematical problem solving, formal bounds, and theorem proving. | Math prompts (e.g., *"prove theorem X"*, *"verify mathematical bounds"*). |
| **Math / Proof (Large Team)** | High-scale tournament networks for hard conjectures and combinatorial search. | Explicit scale signal (e.g., *"Use a very large team of agents"*). |
...
Once approved, Sentinel hands off the project to the Project Orchestrator, which coordinates the team autonomously without needing step-by-step prompts.
...
For example, if you rule out all four shortcuts, the run uses `benchmark` mode, and the Auditor enforces strict verification rules: disallowing mocked test passes, validating ground-truth evaluations against real system execution.
...
- **Exclusive File Ownership**: To prevent merge conflicts and race conditions, the Project Orchestrator assigns specific files to individual Workers. Multiple workers never edit the same file at the same time.
...
- **Autonomous Execution with Guardrails**: Once you approve the initial prompt, the team works through milestones autonomously, with the Auditor and Success Auditor verifying results before final handoff.
...
- [Boost deep reasoning (`/boost`)](https://antigravity.google/docs/boost): Explore on-demand deep reasoning for interactive pair programming.
- [Subagents architecture](https://antigravity.google/docs/subagents): Learn how background subagents execute parallel workstreams.
- [Slash commands overview](https://antigravity.google/docs/slash-commands): Explore the complete catalog of slash commands in Antigravity.
- [Implementation plans (`/plan`)](https://antigravity.google/docs/implementation-plan): Master reviewable planning artifacts and structured workflows.
- [Rules and workflows (`/learn`)](https://antigravity.google/docs/rules-workflows): Persist project-wide patterns and conventions.
- [Antigravity 2.0 Overview](https://antigravity.google/docs/overview): Explore core features and workflows in Antigravity 2.0.
