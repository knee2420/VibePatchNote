...
note: "This URL serves the shared Antigravity MCP guide covering all Antigravity surfaces; see the 'Antigravity CLI' section for CLI-specific configuration."
---
...
**Note:** When declaring remote SSE, Streamable HTTP, or websocket-based MCP connections, you must define the `serverUrl` field. Legacy fields like `url` or `httpUrl` are not supported.
