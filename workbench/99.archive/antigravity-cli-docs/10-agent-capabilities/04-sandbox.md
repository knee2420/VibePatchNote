...
- **`toolPermission`** (string, default: `"request-review"`): Setting this to `"proceed-in-sandbox"` lets sandboxed commands run automatically, while commands that need to run outside the sandbox still prompt for review. See [Settings](https://antigravity.google/docs/cli/settings) for the other modes.
...
The sandbox derives its access boundaries from your **[Permissions](https://antigravity.google/docs/cli/permissions)** configuration:
...
- **[Settings](https://antigravity.google/docs/cli/settings)**: Global CLI preferences and configuration.
