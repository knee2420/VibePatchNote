...
* **구조 (개발 아웃라인 틀):**
  - `apps/web`: 프론트엔드 애플리케이션 (React 19, Vite, Tailwind v4, React Flow, Tiptap) — FSD
  - `apps/api`: 백엔드 애플리케이션 (FastAPI) — 헥사고날 도메인 + Agent Runtime
    - 런타임 데이터는 수명주기 등급으로 분리: `config/` `data/` `cache/` `state/`
      (정본: [`60-data/rule.md`](../../../60-data/rule.md))
    - 저장 레이아웃 변경은 `apps/api/migrations/` 의 명시적 단계로만
  - `packages/scaffold-engine`: **호스트 비의존 문서 분석 엔진** (측정 → 판정 → 조립 → 채점)
  - `packages/document-viewer`: **호스트 비의존적 범용 문서 뷰어 및 플러그인 엔진 (`@vibe/document-viewer`)** (PDF 가로/세로 스프레드 뷰어, 전략 패턴 레지스트리)
  - `packages/tiptap-scaffold`: 스캐폴드 편집기 (`@vibe/tiptap-scaffold`)
